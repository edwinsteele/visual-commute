...
from vcapp.models import InterchangeStation, Line, PartialTrip, Segment,\
    Station, Trip, TripStop
...
class DjangoTransxchangeImporterTestCase(TestCase):
    ...
    def test_correct_counts(self):
        ...
        self.assertEqual(InterchangeStation.objects.count(), 48,
                         "There are %s InterchangeStations" %
                         (InterchangeStation.objects.count(),))
    ...
    def test_segments_are_in_correct_time_order(self):
        for t in Trip.objects.all():
            ...
            for this_segment in t.get_segments():
                ...
                if prev_segment:
                    # Departure time in the previous arrival tripstop is equal
                    #  to the departure time in the departure tripstop in this
                    #  segment
                    self.assertEqual(
                        prev_segment.arrival_tripstop.departure_time,
                        this_segment.departure_tripstop.departure_time
                    )
...
class TestManagerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.central_station = Station.objects.get(
            station_name=self.central_str)
        self.parramatta_station = Station.objects.get(
            station_name=self.parramatta_str)
        self.springwood_station = Station.objects.get(
            station_name=self.springwood_str)
    ...
    def test_find_trips_direct(self):
        ...
        self.assertEquals(len(trips_found_reverse), 0,
                          "Found trips in the wrong direction i.e. departure"
                          "_time at to_station is before departure_time at"
                          "from_station")
    ...
    def test_get_stop_matrix(self):
        ...
        self.assertEquals(matrix_without_to_and_from[-1][0],
                          trip_one_segments[len(trip_one_segments) - 1].
                          arrival_tripstop.station)
        ...
        self.assertEquals(
            combined_matrix[0][0].station_name, "Katoomba Station")
        self.assertEquals(
            combined_matrix[-1][0].station_name, "Gordon Station")
